...
export class TeamsBot extends TeamsActivityHandler {
  ...
  constructor() {
    ...
    this.onMessage(async (turnContext, next) => {
      ...
      //const member = await TeamsInfo.getMember(turnContext, encodeURI('someone@somecompany.com')); - meant to get teams member username, not working
      //from template
  ...
  async authenticateUser(username: string, password: string): Promise<string | null> {
    const wazuhEndpoint = 'https://10.2.184.200:443/security/user/authenticate'; //hardcoded for my wazuh - change this to your own setup
    try {
      ...
      const response = await axios.post(
        ...
        {},
        {
          ...
          auth: {
            username: 'admin',
            password: 'S2o.z?5gX8A*8+AZoaTr4hGWZaUw5a6?',
          },
  ...
  async listAgents(jwtToken: string): Promise<any> {
    const agentsEndpoint = 'https://10.2.184.200:443/agents'; //hardcoded for my wazuh - change this to your own setup
